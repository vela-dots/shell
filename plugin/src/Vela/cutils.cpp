...
void CUtils::getDominantColor(QQuickItem* item, QJSValue callback) {
    this->getDominantColor(item, 128, callback);
}

void CUtils::getDominantColor(QQuickItem* item, int rescaleSize, QJSValue callback) {
    if (!item) {
        qWarning() << "CUtils::getDominantColor: an item is required";
        return;
    }
    ...
    QObject::connect(
        grabResult.data(), &QQuickItemGrabResult::ready, this, [grabResult, rescaleSize, callback, this]() {
            ...
            QThreadPool::globalInstance()->start([grabResult, image, rescaleSize, callback, this]() {
                const QColor color = this->findDominantColor(image, rescaleSize);
...
void CUtils::getDominantColor(const QString& path, QJSValue callback) {
    this->getDominantColor(path, 128, callback);
}

void CUtils::getDominantColor(const QString& path, int rescaleSize, QJSValue callback) {
    if (path.isEmpty()) {
        qWarning() << "CUtils::getDominantColor: given path is empty";
        return;
    }
    ...
    QThreadPool::globalInstance()->start([path, rescaleSize, callback, this]() {
        ...
        if (image.isNull()) {
            qWarning() << "CUtils::getDominantColor: failed to load image" << path;
            return;
        }
...
QColor CUtils::findDominantColor(const QImage& image, int rescaleSize) const {
    if (image.isNull()) {
        qWarning() << "CUtils::findDominantColor: image is null";
        return QColor();
    }
    ...
    return QColor((0xFFu << 24) | dominantColor);
}
